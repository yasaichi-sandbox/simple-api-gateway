...
describe(EffectiveUsersService.name, () => {
  ...
  [
    EffectiveUsersService.prototype.findOneWithLatestPosts,
    EffectiveUsersService.prototype.findOneWithLatestPostsGen,
  ].forEach((method) => {
        ...
        it('should throw `NotFoundException`', async () => {
          Match.value(
            await Effect.runPromiseExit(
              method.call(service, userId).pipe(
      ...
      describe('when the user is found', () => {
        ...
        it('should return a specified user with the latest posts', async () => {
          assert.deepStrictEqual(
            await Effect.runPromise(method.call(service, userId)),
            {
              id: user.id,
